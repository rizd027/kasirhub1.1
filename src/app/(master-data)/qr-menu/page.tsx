...
export default function QrMenuPage() {
  ...
  const getBaseUrl = () => {
    ...
    // If we are on localhost, use the current origin so preview works during development
    if (origin.includes('localhost')) {
      return origin;
    }
    // For mobile/capacitor or specific cases, use configured URL
  ...
  useEffect(() => {
    // 1. First try to get slug from localStorage for immediate feedback
    ...
    // 2. Then fetch from Supabase to ensure we have the most accurate cloud data
