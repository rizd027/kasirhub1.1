...
export function DeleteAllDialog({ productCount, onConfirm, onClose }: DeleteAllDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-card rounded-lg border shadow-2xl w-full max-w-sm p-6 space-y-5">
        <div className="flex items-start justify-between gap-3">
          <div className="p-3 rounded-full bg-red-500/10 shrink-0">
            <AlertTriangle className="h-6 w-6 text-red-500" />
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground" title="Tutup">
            <X className="h-5 w-5" />
          </button>
        </div>
